  ...
  function toggle(){
    ...
    (async ()=>{
      try {
        ...
        const m = JSON.parse(localStorage.getItem('vt:programSubs')||'{}'); m[id]=next; localStorage.setItem('vt:programSubs', JSON.stringify(m))
      } catch(e){}
    })()
  }
  return <button onClick={toggle} className={`px-3 py-2 rounded ${sub? 'bg-green-600 text-white':'bg-gray-200 text-black'}`}>{sub? 'Subscribed':'Subscribe'}</button>
}
